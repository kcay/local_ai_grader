...
public static function grade_submission($submission, $assignment) {
    ...
    try {
        ...
        if (!$submission || !$assignment) {
            throw new \Exception('Invalid submission or assignment object');
        }
        
        // Verify IDs match
        if ($submission->assignment != $assignment->id) {
            throw new \Exception("Submission assignment ID ({$submission->assignment}) does not match assignment ID ({$assignment->id})");
        }
        ...
        if (!isset($assignment->cmid)) {
            ...
            if ($cm) {
                ...
                debugging("Found course module ID: {$cm->id}", DEBUG_DEVELOPER);
            } else {
                throw new \Exception("Could not find course module for assignment ID: {$assignment->id}");
            }
        }
        ...
        if (!isset($assignment->contextid)) {
            if (isset($assignment->cmid)) {
                ...
                debugging("Found context ID: {$context->id}", DEBUG_DEVELOPER);
            }
        }
        
        // Verify assignment exists in database
        $db_assignment = $DB->get_record('assign', ['id' => $assignment->id]);
        if (!$db_assignment) {
            throw new \Exception("Assignment with ID {$assignment->id} not found in database");
        }
        
        // Verify submission exists and belongs to assignment
        $db_submission = $DB->get_record('assign_submission', [
            'id' => $submission->id,
            'assignment' => $assignment->id
        ]);
        if (!$db_submission) {
            throw new \Exception("Submission with ID {$submission->id} not found or doesn't belong to assignment {$assignment->id}");
        }
        
        debugging("Database verification passed", DEBUG_DEVELOPER);
        ...
        debugging("AI response processed - raw grade: {$grading_result['grade']}", DEBUG_DEVELOPER);
        ...
        debugging("Leniency applied ($leniency_level) - adjusted grade: $adjusted_grade", DEBUG_DEVELOPER);
...
private static function send_grading_notification($assignment, $submission, $grade, $feedback) {
    ...
    $message->userfrom = $USER; // The grader (teacher/system)
